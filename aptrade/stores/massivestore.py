...
from aptrade.utils import UTC, AutoDict
...
class MassiveStore(metaclass=MetaSingleton):
    ...
    def getdata(cls, *args, **kwargs):
        ...
        # Provide a clearer error when DataCls wasn't registered yet
        if cls.DataCls is None:
            # Try a lazy import of the feed module that should register the DataCls
            try:
                # Import here to avoid top-level import cycles
                import importlib

                # Common location for the feed that registers this store's DataCls
                importlib.import_module("backtrader.feeds.massivedata")
            except Exception:
                # Ignore import errors here; we'll raise a clearer error below
                pass
            ...
            if cls.DataCls is None:
                raise RuntimeError(
                    (
                        ...
                        "This usually means the corresponding feed module was not imported. "
                        "Import 'backtrader.feeds.massivedata' (or the correct feed) "
    ...
    def __init__(self):
        super(MassiveStore, self).__init__()
        ...
        self.conn = RESTClient(self.p.api_key)
