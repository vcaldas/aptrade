...
from .momentum import RateOfChange100
...
class KnowSureThing(Indicator):
    ...
    params = (
        ...
        ("_rmovav", MovingAverageSimple),
        ("_smovav", MovingAverageSimple),
    )

    plotinfo = dict(plothlines=[0.0])

    def __init__(self):
        rcma1 = self.p._rmovav(RateOfChange100(period=self.p.rp1), period=self.p.rma1)
        rcma2 = self.p._rmovav(RateOfChange100(period=self.p.rp2), period=self.p.rma2)
        rcma3 = self.p._rmovav(RateOfChange100(period=self.p.rp3), period=self.p.rma3)
        rcma4 = self.p._rmovav(RateOfChange100(period=self.p.rp4), period=self.p.rma4)
        self.l.kst = sum(
            [
                rfi * rci
