...
class GapPercent(Indicator):
    """
    Measures the gap between current high vs previous close with respect to that
    of period bars ago
    """
    ...
    # Fancy plotting name
    plotlines = dict(gapchange=dict(_name="%change"))

    # update value to standard for Moving Averages
    params = dict(period=1, threshold=1.0)

    def __init__(self):
        self.addminperiod(self.params.period)
        self.lines.gapchange = self.data.high / self.data.close(-self.p.period) - 1.0

        super(GapPercent, self).__init__()
