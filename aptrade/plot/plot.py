...
class Plot_OldSync(with_metaclass(MetaParams, object)):
    params = (("scheme", PlotScheme()), ("tz", None))
    ...
    def plot(
        ...
        tz = self._tz
        if isinstance(tz, int) and strategy.datas:
            idx = tz
            if 0 <= idx < len(strategy.datas):
                tz = getattr(strategy.datas[idx], "_tz", None)
            else:
                tz = None

        if tz is None and strategy.datas:
            tz = getattr(strategy.datas[0], "_tz", None)
        self._tz_current = tzparse(tz)

        self.pinf = PInfo(self.p.scheme)
        self.pinf.tz = self._tz_current
    ...
    def setlocators(self, ax):
        ...
        tz = getattr(self.pinf, "tz", None)

        fordata = MyDateFormatter(self.pinf.xreal, fmt=fmtdata, tz=tz)
        for dax in self.pinf.daxis.values():
            dax.fmt_xdata = fordata

        # Major locator / formatter
        locmajor = loc.AutoDateLocator(self.pinf.xreal, tz=tz)
        ax.xaxis.set_major_locator(locmajor)
        if self.pinf.sch.fmt_x_ticks is None:
            autofmt = loc.AutoDateFormatter(self.pinf.xreal, locmajor, tz=tz)
        else:
            autofmt = MyDateFormatter(
                self.pinf.xreal,
