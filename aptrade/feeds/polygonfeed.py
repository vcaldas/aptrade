...
from aptrade.stores import PolygonStore
from aptrade.utils.py3 import with_metaclass


class MetaPolygonFeed(DataBase.__class__):
    def __init__(cls, name, bases, dct):
        """Class has already been created ... register"""
        # Initialize the class
        super(MetaPolygonFeed, cls).__init__(name, bases, dct)

        # Do not register here to avoid circular import / timing issues.
        # Registration will be performed at module level after the
        # PolygonFeed class is defined.
        PolygonStore.DataCls = cls


class PolygonFeed(with_metaclass(MetaPolygonFeed, DataBase)):
    ...
    def setenvironment(self, env):
        ...
        belongs to"""
        super(PolygonFeed, self).setenvironment(env)
            ...
            if self._state == self._ST_LIVE:
                if self._timeframe == bt.TimeFrame.Ticks:
                    ...
                    if self.p.debug:
                        print("----     LOAD    ----")
                        print(
                            "{} Load OHLCV Returning: {}".format(datetime.utcnow(), ret)
                        )
                    return ret
    ...
    def _fetch_ohlcv(self, fromdate=None):
        ...
        while True:
            ...
            if self.p.debug:
                # TESTING
                since_dt = (
                    datetime.utcfromtimestamp(since // 1000)
