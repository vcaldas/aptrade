...
from aptrade import analyzers

from . import indicator, linebuffer, nplot, observers
from .brokers import BackBroker
from .metabase import MetaParams
from .nplot.utils import gen_timestamp
...
class Cerebro(metaclass=MetaParams):
    ...
    def __init__(self, broker=None, datas=None):
        ...
        self.strats = list()
        self.runstrats = list()  # Will be populated after run()
    ...
    def statistics(self) -> pd.Series:
        flat_runstrats = [strat for stratlist in self.runstrats for strat in stratlist]
        print("Flat runstrats", flat_runstrats)
    ...
    def show_report(self, name=None, filename=None, show=True):
        ...
        report_gen.report(
            name=name, performance=eq, show=show, filename=filename, iplot=iplot
        )
    ...
    def run(self, **kwargs):
        ...
        if not self.datas:
            print("Cerebro.run: No data feeds in the system")
            return []  # nothing can be run

        allowed = set(self.params._getkeys())
