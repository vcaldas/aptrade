...
class MassiveAPIClient:
    ...
    def __init__(
        ...
        logger.debug(
            f"[MassiveAPIClient] API key first 5 chars: {api_key[:5] if api_key and len(api_key) >= 5 else 'N/A'}"
        )
        logger.debug(f"[MassiveAPIClient] use_mock: {use_mock}")
    ...
    def fetch_snapshots(self) -> List[TickerSnapshot]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"[MassiveAPIClient] API fetch attempt {attempt + 1}/{self.max_retries} failed: {e}"
                )
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.critical(
                        f"[MassiveAPIClient] All API fetch attempts failed after {self.max_retries} tries"
                    )
                    raise
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if "authorization" in str(e).lower() or "malformed" in str(e).lower():
                    ...
                    logger.error(
                        f"[MassiveAPIClient] API key format: {type(self.api_key)}"
                    )
                    logger.error(f"[MassiveAPIClient] Error message: {str(e)}")
...
def extract_ohlcv(snapshot: TickerSnapshot) -> Optional[Agg]:
    ...
    if minute_bar is not None and hasattr(minute_bar, "close"):
        ...
        logger.debug(f"{snapshot.ticker}: Using prev_day data for OHLCV")
...
class MassiveLiveData(bt.DataBase):
    ...
    def __init__(self):
        ...
        self.tradecontract = Contract()
        self.tradecontract.symbol = self.p.ticker or ""
        self.tradecontract.secType = "STK"
        self.tradecontract.exchange = "SMART"
        self.tradecontract.currency = "USD"
    ...
    def _load(self):
        ...
        For live feeds - NON-BLOCKING implementation:
        - Returns True if a bar was loaded
        - Returns None if no bar available right now (feed stays alive)
        - Returns False if feed should stop

        This method is called repeatedly by backtrader's event loop.
        It returns immediately without blocking so the loop can continue.
        """
        # Check if we should stop
        if self._should_stop.is_set():
            logger.debug(f"{self.p.ticker}: Stop signal received")
            return False

        # Try to get a bar from queue WITHOUT BLOCKING
        if self._bar_queue.empty():
            # No bar available - return None
            # This tells backtrader: "feed is alive but no data yet"
            # Backtrader will call _load() again on the next event loop iteration
            return None
...
class MassiveFeedManager:
    ...
    def __init__(
        ...
        """
        logger.info(
            f"[MassiveFeedManager] Initializing with api_key length: {len(api_key) if api_key else 0}"
        )
        logger.info(f"[MassiveFeedManager] use_mock: {use_mock}")
    ...
    def create_feeds(self, cerebro: bt.Cerebro) -> int:
        ...
        try:
            if self.gappers:
                snapshots = self.api_client.fetch_gappers()
            else:
        ...
        for snapshot in snapshots:
            ...
            if self.symbol_filter and not self.symbol_filter(snapshot):
                logger.debug(f"Filtered {snapshot.ticker}")
                continue
        ...
        logger.info(
            "Feed registration complete: cerebro.datas=%s sample=%s",
            len(cerebro.datas),
            sample_tickers,
        )
    ...
    def start_polling(self):
        ...
        if self._running and self._polling_thread and self._polling_thread.is_alive():
    ...
    def _polling_loop(self):
        ...
        self._running = False
        logger.info(f"[POLLING-STOP] Polling loop stopped after {cycle} cycles")

    def is_polling_running(self) -> bool:
        """Return True when the polling thread is expected to be actively running."""
        return bool(
            self._running and self._polling_thread and self._polling_thread.is_alive()
        )
    ...
    def _fetch_and_update(self):
        ...
        logger.info(
            f"[heartbeat] Pull cycle #{cycle} started (tracked feeds: {self.get_feed_count()})"
        )
        ...
        try:
            ...
            logger.error(f"[heartbeat] Pull cycle #{cycle} failed: {e}")
            return
...
def setup_massive_live_feeds(
    ...
    def symbol_filter(snapshot) -> bool:
        ...
        if not hasattr(snapshot, "ticker"):
            logger.info(f"{snapshot.ticker} - Filtered lacking ticker")
            return False
        ...
        if min_price is not None and close < min_price:
            logger.debug(f"{snapshot.ticker} - Below min price")
            return False

        if max_price is not None and close > max_price:
            logger.debug(f"{snapshot.ticker} - Above max price")
            return False
    ...
    manager = MassiveFeedManager(
        ...
        symbol_filter=symbol_filter if (min_volume or min_price or max_price) else None,
        use_mock=use_mock,
    ...
    if auto_start:
        logger.info("[setup_massive_live_feeds] Starting polling with auto_start=True")
        manager.start_polling()
        logger.info(
            "[setup_massive_live_feeds] Polling started: %s",
            manager.is_polling_running(),
        )
